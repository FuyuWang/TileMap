...
class Actor(nn.Module):
    ...
    def get_remain_buffer_size(self, cur_buffer_level, trg_seq_disorder, order_action, is_cur):
        ...
        if cur_buffer_level == 2:
            ...
            output_tile_M = H * M_temp * N
            output_tile_H = H_temp * M * N

            input_tile = H_temp * M_temp * K_temp
            weight_tile = H_temp * K_temp * N
            output_tile = H_temp * M_temp * N
            ...
            H_coef = input_tile_H + weight_tile_H + output_tile_H
            M_coef = M*(H_temp * K_temp + H_temp * N)
            K_coef = K*(H_temp * M_temp + H_temp * N)
            N_coef = N*(H_temp * M_temp + H_temp * K_temp)
