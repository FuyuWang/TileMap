...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.order_size = 5 + 1
        self.order_emb = nn.Embedding(self.order_size, d_word_vec, padding_idx=self.trg_pad_idx)
        self.tile_size = 30 + 1  # 12+1(start)
        # self.tile2_emb = nn.Embedding(self.tile_size, d_word_vec, padding_idx=pad_idx)
        # self.tile3_emb = nn.Embedding(self.tile_size, d_word_vec, padding_idx=pad_idx)
        # self.tile5_emb = nn.Embedding(self.tile_size, d_word_vec, padding_idx=pad_idx)
        # self.tile7_emb = nn.Embedding(self.tile_size, d_word_vec, padding_idx=pad_idx)
        self.tile_emb_list = nn.ModuleList([nn.Embedding(self.tile_size, d_word_vec, padding_idx=self.trg_pad_idx)
                                            for _ in range(num_primes)])
        self.parallel_size = 2 + 1  # 0, 1
        self.sp_tile2_emb = nn.Embedding(self.tile_size, d_word_vec, padding_idx=self.trg_pad_idx)
        self.d_model = d_model
        self.num_primes = num_primes
        ...
        self.tile_prj_list = nn.ModuleList([nn.Linear(d_model, self.tile_size, bias=False)
                                            for _ in range(num_primes)])
        self.sp_tile2_prj = nn.Linear(d_model, self.tile_size, bias=False)
        ...
        if trg_emb_prj_weight_sharing:
            ...
            for i in range(num_primes):
                self.tile_prj_list[i].weight = self.tile_emb_list[i].weight
            self.sp_tile2_prj.weight = self.sp_tile2_emb.weight
    ...
    def forward(self, trg_seq):
        ...
        dec_output = self.decoder(dec_output).mean(dim=1)
