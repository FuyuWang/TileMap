    ...
    for architecture in architectures:
        ...
        for llm in llms:
            ...
            for input_size in input_sizes:
                ...
                layer2chkpt = {}
                for i, layer in enumerate(layers[2:3]):
                    print(architecture, llm, input_size, i, layer)
                    set_seed(opt.seed)
                    report_dir = os.path.join(opt.report_dir, architecture, 'fc_fitness_{}'.format(opt.fitness1),
                    ...
                    if layer_to_key in layer2chkpt:
                        ...
                        actor = Actor(opt.d_model, opt.d_inner, opt.n_layers, opt.n_head, opt.d_k, opt.d_v,
                                      ...
                                      problem['problem']['instance'], env.prime2idx, tile_budgets).to(device)
                        actor.train()
