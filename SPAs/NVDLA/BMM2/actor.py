...
class Actor(nn.Module):
    def __init__(self, d_model, d_inner, n_layers, n_head, d_k, d_v, buf_spmap_cstr, buffer_size_list, steps_per_level,
                 problem_instance, prime2idx, tile_budgets):
    ...
    def get_remain_buffer_size(self, cur_buffer_level, trg_seq_disorder, order_action, is_cur):
        ...
        K, N, M, H = torch.unbind(tiles, dim=1)
        ...
        if cur_buffer_level == 3:  # SMEM
            ...
            H_temp = 1
            for k, v in self.prime2idx.items():
                K_temp *= torch.pow(int(k), self.tile_budgets[:, 0, v])
                M_temp *= torch.pow(int(k), self.tile_budgets[:, 2, v])
                H_temp *= torch.pow(int(k), self.tile_budgets[:, 3, v])
            input_tile_K = H_temp * M_temp * K
            input_tile_M = H_temp * M * K_temp
            input_tile_H = H * M_temp * K_temp

            weight_tile_K = H_temp * K * N
            weight_tile_H = H * K_temp * N

            input_tile = H_temp * M_temp * K_temp
            weight_tile = H_temp * K_temp * N
            ...
            H_coef = input_tile_H + weight_tile_H
            M_coef = M * H_temp * K_temp
            K_coef = K * (H_temp * M_temp + H_temp * N)
            N_coef = N * H_temp * K_temp
        else:
            if cur_buffer_level == 1:   # LRF
                input_tile = H * M * K
        ...
        coef_arr = torch.stack([K_coef, N_coef, M_coef, H_coef], dim=1)[
            np.arange(batch_size), order_action]
        sub_arr = torch.stack([K_sub, N_sub, M_sub, H_sub], dim=1)[
            np.arange(batch_size), order_action]
    ...
    def forward(self, trg_seq, trg_mask, order_mask, tile_remain_budgets, tile_masks, parallel_mask,
        ...
        for later_level in range(cur_buffer_level + 1, len(self.buffer_size_list)):
        ...
        # print('mode: ', mode, tile2_mask[0], tile2_max[0], sp_tile2_action[0])
        ...
        # print('tile2_action: ', mode, tile2_max[0], tile2_action[0])
