...
class TimeloopEnv(object):
    ...
    def init_tp_tile_size(self):
        series =  [f'{note}={1}' for note in 'KNMH']
        return ' '.join(series)
    ...
    def run_timeloop(self, dimension, sol,
        ...
        if stderr:
            ...
            try:
                ...
                stats = run_config(self.pool_path[pool_idx])
    ...
    def get_stats(self, stats):
        return [stats.cycles * stats.energy * 1E-6, stats.cycles, stats.energy * 1E-6, stats.utilization,
                stats.energy / stats.algorithmic_compute, stats.energy / stats.actual_compute, stats.area * 1E-6]
