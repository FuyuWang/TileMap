...
class Environment(object):
    def __init__(self, in_config_dir='./in_config', arch_file='arch', fitness_obj=['latency'], report_dir='./report',
        ...
        print( self.dimension, self.dimension_prime, self.prime2idx)
        ...
        # BERT:
        self.pre_factors = [9, -1, 7, 1]
        self.tile_remain_budgets = copy.deepcopy(self.tile_budgets)
        self.tile_remain_budgets[:, 0, 0] = self.pre_factors[0]
        self.tile_remain_budgets[:, 2, 0] = self.pre_factors[2]
        self.tile_remain_budgets[:, 3, 0] = self.pre_factors[3]
        ...
        for k, _ in enumerate('KNMH'):
            for j in range(0, self.num_primes):
                tile_remain_budget = self.tile_remain_budgets[:, k, j]
                for i in range(1, self.max_tile + 1):
                    self.tile_masks[np.arange(self.batch_size), k, j, tile_remain_budget + i] = float('-inf')
        ...
        if self.cur_buffer_level < self.num_buf_levels:
            ...
            for i in range(1, self.max_tile + 1):
                for j in range(0, self.num_primes):
                    ...
                    self.tile_masks[
                        np.arange(self.batch_size), order_action, j, tile_remain_budget + i] = float('-inf')
        else:
            ...
            if self.mode % self.steps_per_level > 0:
                self.final_trg_seq[np.arange(self.batch_size), self.mode - self.steps_per_level, 1: self.num_primes + 1] += tile_actions
                self.tile_remain_budgets[np.arange(self.batch_size), order_action] -= tile_actions

            tile_remain_budgets2 = copy.deepcopy(self.tile_remain_budgets)

            tile_remain_budgets2[:, 0, 0] = copy.deepcopy(self.tile_budgets[:, 0, 0]) - self.pre_factors[0]
            tile_remain_budgets2[:, 2, 0] = copy.deepcopy(self.tile_budgets[:, 2, 0]) - self.pre_factors[2]
            tile_remain_budgets2[:, 3, 0] = copy.deepcopy(self.tile_budgets[:, 3, 0]) - self.pre_factors[3]

            tile_actions = tile_remain_budgets2[np.arange(self.batch_size), order_action]
