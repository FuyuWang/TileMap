    ...
    def get_remain_buffer_size(self, cur_buffer_level, trg_seq_disorder, order_action, is_cur):
        ...
        if cur_buffer_level == 3:  # SMEM
            ...
            for k, v in self.prime2idx.items():
                ...
                N_temp *= torch.pow(int(k), self.tile_budgets[:, 1, v])
            input_tile_K = H * M * K
            weight_tile_K = H * K * N_temp
            weight_tile_N = H * K_temp * N

            input_tile = H * M * K_temp
            weight_tile = H * K_temp * N_temp
