...
class Environment(object):
    def __init__(self, in_config_dir='./in_config', arch_file='arch', fitness_obj=['latency'], report_dir='./report',
        ...
        self.pre_factors = [self.tile_budgets[0, 0, 0], self.tile_budgets[0, 1, 0], -5, -1]
        self.tile_remain_budgets = copy.deepcopy(self.tile_budgets)
        self.tile_remain_budgets[:, 0, 0] = self.pre_factors[0]
        self.tile_remain_budgets[:, 1, 0] = self.pre_factors[1]
    ...
    def step(self, actions):
        if self.cur_buffer_level < self.num_buf_levels:
            ...
            if self.mode % self.steps_per_level <= 1:
                self.final_trg_seq[np.arange(self.batch_size), self.mode - self.steps_per_level,
                1: self.num_primes + 1] += tile_actions
                self.tile_remain_budgets[np.arange(self.batch_size), order_action] -= tile_actions
            ...
            tile_remain_budgets2[:, 1, 0] = copy.deepcopy(self.tile_budgets[:, 1, 0]) - self.pre_factors[1]
